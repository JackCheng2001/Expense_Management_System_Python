...
class ExpensesLoader(object):
    ...
    def import_expenses(self, expenses, file):
        ...
        with open(file, 'r') as f:
            ...
            for line in f:
                # Split each line at the colon and remove any leading/trailing whitespace
                parts = line.strip().split(':')
                # Ensure the line has both an expense type and an amount
                if len(parts) == 2 and parts[1]:  # ensure there is an amount
                    # Assign expense type to 'key' and cast the amount to a float, assign to 'value'
                    key, value = parts[0].strip(), float(parts[1].strip())
                    # If the expense type already exists in the dictionary, add to its amount
                    if key in expenses:
                        expenses[key].add_amount(value)
                    # Otherwise, create a new Expense object for the new expense type
                    else:
                        expenses[key] = Expense(key, value)
